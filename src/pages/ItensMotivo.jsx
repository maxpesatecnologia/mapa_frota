...
const EMPTY = { nome: '', numero: '', motivo_id: '' };

const ItensMotivo = () => {
  const { itensMotivoList, motivosList, saveItemMotivo, deleteItemMotivo } = useCadastros();
  ...
  const openEdit = (i) => {
    setForm({ nome: i.nome, numero: i.numero || '', motivo_id: i.motivo_id || '' });
    setEditId(i.id);
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    await saveItemMotivo(form, editId);
    setSaving(false);
    close();
  };

  const handleDelete = async (id) => {
    if (!confirm('Excluir este item de motivo?')) return;
    await deleteItemMotivo(id);
  };

  const filtered = itensMotivoList.filter(i =>
  ...
  return (
    <div style={{ padding: '1.5rem', height: '100%', overflowY: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
        <div>
          <h1 style={{ fontSize: '1.3rem', color: '#1e293b' }}>Itens de Motivo</h1>
          <p style={{ fontSize: '0.8rem', color: '#64748b', marginTop: 2 }}>
            {itensMotivoList.length} item(ns) cadastrado(s)
          </p>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
              {['Nº', 'Item de Motivo', 'Motivo', ''].map(h => (
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={4} style={{ padding: '2rem', textAlign: 'center', color: '#94a3b8', fontSize: '0.875rem' }}>
                {itensMotivoList.length === 0 ? 'Nenhum item cadastrado.' : 'Nenhum resultado.'}
              </td></tr>
            ) : filtered.map((item, i) => (
              <tr key={item.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #f1f5f9' : 'none' }}
                onMouseEnter={ev => ev.currentTarget.style.background = '#fafbfd'}
                onMouseLeave={ev => ev.currentTarget.style.background = 'white'}
              >
                <td style={{ padding: '0.85rem 1rem', color: '#64748b', fontSize: '0.875rem' }}>{item.numero}</td>
                <td style={{ padding: '0.85rem 1rem', fontWeight: 600, fontSize: '0.875rem', color: '#1e293b' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <Tags size={14} color="#94a3b8" /> {item.nome}
                  </div>
                </td>
                <td style={{ padding: '0.85rem 1rem', color: '#64748b', fontSize: '0.875rem' }}>
                  {motivosList.find(m => m.id === item.motivo_id)?.nome || '-'}
                </td>
                <td style={{ padding: '0.85rem 1rem' }}>
                  <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'flex-end' }}>
                    <button onClick={() => openEdit(item)} style={{ padding: '0.35rem', borderRadius: 6, border: 'none', background: '#f1f5f9', color: '#64748b', cursor: 'pointer' }}><Pencil size={14} /></button>
                    <button onClick={() => handleDelete(item.id)} style={{ padding: '0.35rem', borderRadius: 6, border: 'none', background: '#fef2f2', color: '#E30613', cursor: 'pointer' }}><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: 14, padding: '1.75rem', width: '100%', maxWidth: 400, boxShadow: '0 20px 40px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ fontSize: '1.1rem', color: '#1e293b' }}>{editId ? 'Editar Item' : 'Novo Item'}</h2>
              <button onClick={close} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8' }}><X size={20} /></button>
            </div>

            <div style={{ display: 'grid', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 600, color: '#475569', marginBottom: '0.35rem' }}>Número</label>
                <input
                  type="number" value={form.numero} onChange={e => setForm(f => ({ ...f, numero: e.target.value ? Number(e.target.value) : '' }))}
              ...
              <div>
                ...
                <select
                  value={form.motivo_id} onChange={e => setForm(f => ({ ...f, motivo_id: e.target.value }))}
                  style={{ width: '100%', padding: '0.6rem 0.85rem', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: '0.875rem', boxSizing: 'border-box' }}
                >
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 600, color: '#475569', marginBottom: '0.35rem' }}>Nome do Item *</label>
                <input
